...
package com.quizlet.vbmda.common.utils;
...
public class NestedProjections<T> extends MappingProjection<T> {
  ...
  public static <T> NestedProjections<T> of(
      final Class<T> clazz, final String rootAlias, final Path<?>... args) {
    ...
    try {
      ...
      for (final Path<?> arg : root) {
        ...
        if (string.contains(UNDERLINE_PROPERTY)) {
          propertyPath =
              StringUtils.join(
                  ArrayUtils.remove(StringUtils.split(string, UNDERLINE_PROPERTY), 0),
                  DOT_PROPERTY);
        } else {
          propertyPath =
              StringUtils.join(
                  ArrayUtils.remove(StringUtils.split(string, DOT_PROPERTY), 0), DOT_PROPERTY);
        }
      ...
      for (final Path<?> arg : other) {
        ...
        if (string.contains(UNDERLINE_PROPERTY)) {
          propertyPath =
              StringUtils.join(StringUtils.split(string, UNDERLINE_PROPERTY), DOT_PROPERTY);
        } else {
      ...
      throw new RuntimeException(
          "Unexpected error while initializing nested projections.", exception);
    }
  }
    ...
    for (int i = 0; i < fieldNames.length - 1; i++) {
      ...
      if (value == null) {
        final PropertyDescriptor propertyDescriptor =
            PropertyUtils.getPropertyDescriptor(obj, nestedProperty.toString());
